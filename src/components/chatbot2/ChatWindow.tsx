...
const ChatWindow = ({ logo, onClose }: ChatWindowProps) => {
  // const [, setDeluxAdult] = useState(0);
  ...
  const [GatewayConnected] = useState({
    Type: "Razorpay",
  ...
  const [hid, setHid] = useState("56369483");
  ...
  const PaymentSuccessFull = async (responseOrderId: string, payid: any) => {
    const response = await fetch(
      `${"https://nexon.eazotel.com"}/booking/update`,
      {
        method: "POST",
        headers: {
  ...
  const HandlePaymentRazorpay = (
    my_api_order_id: any,
    amnt: number,
    status: any
  ) => {
    console.log("In handle payment razorpay", my_api_order_id, amnt, status);

    try {
      const mockOrderData = {
        amount: amnt * 100, // Convert amount to paise (assuming INR)
        orderId: my_api_order_id, // Generate a unique order ID
      };
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          setPayment({
            ...
            PayStatus: status,
            Delux: String(Delux),
            Sd: String(SuperDelux),
            Suite: String(Suite),
            Premium: String(Premium),
            PremiereRetreat: String(PremiereRetreat),
            EliteSuite: String(EliteSuite),
            GrandDeluxe: String(GrandDeluxe),
            ImperialSuite: String(ImperialSuite),
            SupremeRetreat: String(SupremeRetreat),
            RoyalDeluxe: String(RoyalDeluxe),
            PrestigeSuite: String(PrestigeSuite),
            ExclusiveRetreat: String(ExclusiveRetreat),
            MealPlan: "",
            Mealprice: "",
  ...
  const CreateBooking = async () => {
    if (
      ...
      if (json.Status === true) {
        const my_api_order_id = json.order_id;
        console.log(my_api_order_id);
        // setRedirectLink(json.redirectLink);

        console.log(
          "Call handle function here with order id ",
          my_api_order_id
        );
        await HandlePaymentRazorpay(my_api_order_id, total_cost, "SUCCESS");
  ...
  const totalCost = (currency: any, ratesChange: any) => {
    let deluxcost, sdcost, suitecost, premiumcost, premiereretreatcost;
    let elitesuitecost, granddeluxecost, imperialsuitecost;
    let supremeretreatcost,
      royaldeluxecost,
      prestigesuitecost,
      exclusiveretreatcost;
    try {
      deluxcost = Delux * Number(ratesChange["1"]["Price"]);
    } catch {
      deluxcost = 0;
    }
    try {
      sdcost = SuperDelux * Number(ratesChange["2"]["Price"]);
    } catch {
      sdcost = 0;
    }
    try {
      suitecost = Suite * Number(ratesChange["3"]["Price"]);
    } catch {
      suitecost = 0;
    }
    try {
      premiumcost = Premium * Number(ratesChange["4"]["Price"]);
    } catch {
      premiumcost = 0;
    }
    try {
      premiereretreatcost = PremiereRetreat * Number(ratesChange["5"]["Price"]);
    } catch {
      premiereretreatcost = 0;
    }
    try {
      elitesuitecost = EliteSuite * Number(ratesChange["6"]["Price"]);
    } catch {
      elitesuitecost = 0;
    }
    try {
      granddeluxecost = GrandDeluxe * Number(ratesChange["7"]["Price"]);
    } catch {
      granddeluxecost = 0;
    }
    try {
      imperialsuitecost = ImperialSuite * Number(ratesChange["8"]["Price"]);
    } catch {
      imperialsuitecost = 0;
    }
    try {
      supremeretreatcost = SupremeRetreat * Number(ratesChange["9"]["Price"]);
    } catch {
      supremeretreatcost = 0;
    }
    try {
      royaldeluxecost = RoyalDeluxe * Number(ratesChange["10"]["Price"]);
    } catch {
      royaldeluxecost = 0;
    }
    try {
      prestigesuitecost = PrestigeSuite * Number(ratesChange["11"]["Price"]);
    } catch {
      prestigesuitecost = 0;
    }
    try {
    ...
    if (currency == "INR") {
      ...
      //   tax = 0;
      // }
      const totalcost = Number(cost) + Number(tax);
      return totalcost;
    }
  };
                      ...
                      >
                        <div className="w-70 flex flex-col gap-3">
                          ...
                          <div className="flex w-full justify-end  items-center">
                            <button
                              disabled={msg?.disabled}
                              className={`text-white bg- py-1.5 px-6 text-sm tracking-wide rounded-full ${
                                msg?.disabled ? "opacity-40" : "cursor-pointer"
                              }`}
                              style={{
                                background: themeStyle?.BackgroundColor
                                  ? themeStyle?.BackgroundColor
                    ...
                    {msg?.roomSummary && (
                      <div className=" flex flex-col gap-10">
                        <div className="flex flex-col gap-3 ">
                          ...
                          >
                            ...
                            >
                              ...
                              <p>
                                ...
                                {hotelDetails?.hotels?.[hid]?.pincode}
                              </p>
                              <p>
                          ...
                          {/* <div
                            ref={pdfRef}
                            className=" flex flex-col gap-5 text-md bg-gray-100 text-gray-600 w-[90%] p-5 rounded-br-3xl rounded-bl-3xl rounded-tr-3xl"
                          >
...
//     // console.log(
//     //   "HandlePaymentRazorpay called with orderID:",
//     //   orderID,
//     //   "and amount:",
//     //   amnt
//     // );
