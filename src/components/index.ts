import Section from "./Section";
import Container from "./Container";
import Banner from './Banner';
import NavBar from './NavBar';
import Card from './Card';
import Location from "./Location";
import Footer from './Footer';
import LazyLoadedMap from './LazyLoadedMap';
import EmbeddedMap from './EmbeddedMap';
import Whatsapp from './WhatsApp';
import Call from './Call';
import SectionHeading from './SectionHeading';
import TwoColGridCard from './TwoColGridCard';
import FullScreenImageViewPopUP from './FullScreenImageViewPopUP';
import Form from './Form';
// import VideoSection from './VideoSectionProps';



export { Section, FullScreenImageViewPopUP, Form, SectionHeading, TwoColGridCard, Container, Banner, Location, NavBar, Card, Footer, LazyLoadedMap, EmbeddedMap, Whatsapp, Call }